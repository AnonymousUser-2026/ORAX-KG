...
def compute_similarities(
    extracted_embs: List[Dict],
    ontology_embs: List[Dict],
) -> Dict[str, torch.Tensor]:
    """
    Compute pairwise cosine similarity matrices between extracted triples
    and ontology patterns across four independent views.

    Args:
        extracted_embs: Output of TripleEmbedder.embed_triples()
        ontology_embs: Output of TripleEmbedder.embed_ontology()

    Returns:
        Dict with keys 'triple', 'relation', 'subject', 'object',
        each a [N_extracted, N_ontology] similarity tensor
    """
    device = extracted_embs[0]["triple_emb"].device
    zero   = torch.zeros_like(extracted_embs[0]["relation_emb"])
    ...
    ext_triple = _stack(extracted_embs, "triple_emb")
    ext_rel    = _stack(extracted_embs, "relation_emb")
    ext_sub    = _stack(extracted_embs, "subj_type_emb")
    ext_obj    = _stack(extracted_embs, "obj_type_emb")
